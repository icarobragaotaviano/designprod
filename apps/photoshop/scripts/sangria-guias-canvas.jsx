/**
 * @ibd-id photoshop/sangria-guias-canvas
 * @ibd-titulo Sangria por guias e tela
 * @ibd-descricao Acrescenta sangria uniforme à tela, com guias do corte e da borda externa, sem redimensionar a arte.
 * @ibd-app photoshop
 * @ibd-versao 1.0.0
 * @ibd-tags sangria, guias, canvas, arte-final
...
if (!app.documents.length) { alert("Abra o documento no tamanho final de corte."); return; }
var source = app.activeDocument, target = null, copied = false;
try {
    if (PS.hasArtboards(source)) {
        throw new Error("Esta vers\u00e3o de sangria trabalha com uma tela \u00fanica.\nDocumentos com pranchetas precisam ser tratados por prancheta.");
    }
    var width, height, resolution = source.resolution;
    PS.pixels(source, function () { width = source.width.as("px"); height = source.height.as("px"); });
    var w = new Window("dialog", "Sangria por guias + tela | Photoshop");
    w.orientation = "column"; w.alignChildren = "fill"; w.spacing = 12; w.margins = 18;
    G.message(w, "Use o documento no tamanho de corte. A sangria ser\u00e1 acrescentada igualmente nos quatro lados.", 38);
    var row = w.add("group");
    row.add("statictext", undefined, "Sangria em cada lado:");
    var value = row.add("edittext", undefined, "3"); value.characters = 9;
    var unit = row.add("dropdownlist", undefined, ["mm", "cm", "px", "pt"]); unit.selection = 0;
    var duplicate = w.add("checkbox", undefined, "Aplicar em uma c\u00f3pia do documento"); duplicate.value = true;
    G.message(w, "Ser\u00e3o criadas guias do corte e da nova borda.\nA arte mant\u00e9m tamanho e resolu\u00e7\u00e3o; prolongue os fundos depois.", 40);
    G.message(w, "Com Plano de Fundo, a nova faixa pode receber cor; sem ele, transpar\u00eancia. Cada execu\u00e7\u00e3o acrescenta uma nova sangria.", 40);
    var preview = G.message(w, "", 76);
    var buttons = w.add("group"); buttons.alignment = "right";
    buttons.add("button", undefined, "Cancelar", {name:"cancel"});
    var ok = buttons.add("button", undefined, "Criar sangria", {name:"ok"});
    var config = null;
    function read() { return G.bleed(width, height, G.number(value.text), unit.selection.text, resolution); }
    function update() {
        try {
            var c = read();
            preview.text = "Tela atual: " + width + " \u00d7 " + height + " px | " + G.fmt(resolution) + " ppi\n" +
                "Nova tela: " + c.width + " \u00d7 " + c.height + " px\n" +
                "Sangria aplicada: " + c.px + " px = " + G.fmt(c.mm, 4) + " mm por lado.\n" +
                "Arredondamento para cima ao pixel inteiro.";
            ok.enabled = true;
        } catch (e) { preview.text = e.message; ok.enabled = false; }
    }
    value.onChanging = update; unit.onChange = update;
    ok.onClick = function () {
        try { config = read(); config.duplicate = duplicate.value; w.close(1); } catch (e) { alert(e.message); }
    };
    update(); w.center(); value.active = true;
    if (w.show() !== 1 || !config) { return; }
    if (config.duplicate) {
        var newName = source.name.replace(/\.[^\.]+$/, "") + "_sangria";
        target = source.duplicate(newName, false); copied = true;
    } else { target = source; }
    app.activeDocument = target;
    var created = 0;
    PS.history(target, "Sangria por guias + tela", function () {
        PS.pixels(target, function () {
            target.resizeCanvas(UnitValue(config.width,"px"), UnitValue(config.height,"px"), AnchorPosition.MIDDLECENTER);
            if (Math.abs(target.width.as("px")-config.width) > 0.01 ||
                Math.abs(target.height.as("px")-config.height) > 0.01) {
                throw new Error("O Photoshop n\u00e3o aplicou o tamanho de tela esperado.");
            }
            // As guias do corte s\u00e3o calculadas na tela final: independe do deslocamento
            // que o Photoshop aplicar \u00e0s guias que j\u00e1 existiam no documento.
            created = PS.addGuides(target, G.plan(config.rects));
        });
    });
    alert("Sangria criada: " + G.fmt(config.mm, 4) + " mm por lado.\n" +
        "Nova tela: " + config.width + " \u00d7 " + config.height + " px.\n" + created + " guia(s) nova(s).\n\n" +
        "Agora prolongue fundos e imagens at\u00e9 a borda externa e salve o PSD.\n" +
        "As guias n\u00e3o s\u00e3o marcas de corte impressas." + (copied ? "\nO resultado est\u00e1 em uma c\u00f3pia ainda n\u00e3o salva." : ""));
} catch (e) {
    var failure = e;
    if (copied && target) {
        try { target.close(SaveOptions.DONOTSAVECHANGES); app.activeDocument = source; }
        catch (closeError) { failure = new Error(e.message + "\nA c\u00f3pia tempor\u00e1ria ficou aberta. Verifique-a antes de salvar."); }
    }
    PS.error(failure);
}
